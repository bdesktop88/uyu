...
app.post('/add-redirect', (req, res) => {
  ...
  const encodedDestination = base64Encode(destination);
  ...
  db.get('SELECT value FROM settings WHERE key = ?', ['recaptcha_secret_key'], async (err, row) => {
    ...
    try {
      ...
      db.get(`SELECT destination FROM redirects WHERE token = ?`, [token], (err, row) => {
        ...
        try {
          const decodedDestination = base64Decode(row.destination);
          res.json({ destination: decodedDestination });
        } catch (e) {
          console.error('Decoding failed:', e);
          res.status(500).json({ message: 'Failed to decode destination' });
        }
      });
    } catch (e) {
      console.error(e);
...
app.get('/admin/redirects', (req, res) => {
  db.all('SELECT token, destination FROM redirects', [], (err, rows) => {
    ...
    const decodedRows = rows.map(row => ({
      token: row.token,
      destination: base64Decode(row.destination)
    }));
...
app.post('/admin/update-redirect', (req, res) => {
  ...
  const encodedDestination = base64Encode(destination);
  db.run(`UPDATE redirects SET destination = ? WHERE token = ?`, [encodedDestination, token], function (err) {
